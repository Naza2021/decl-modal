...
export const useTooltipPosition = ({ target, containerId = 'back' }: UseTooltipPositionProps) => {
    ...
    useEffect(() => {
        ...
        const resizeEvent = (_event: any, coornadinatesA: any = 'b', coordinatesB: any = 't') => {
            ...
            if (zoneOut) {
                ...
                generateZoneElement(getOffsets(target) as any, CustomOffsets + zoneOffsets, `${customId}1`)
                generateZoneElement({ ...getOffsets(backTooltip) as any, top: firstResolution.y, left: finalResolution }, CustomOffsets + zoneOffsets, `${customId}2`)
