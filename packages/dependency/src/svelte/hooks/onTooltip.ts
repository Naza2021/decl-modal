...
interface UseTooltipPositionProps {
  ...
  onZoneLeave?: () => void;
  pointTarget?:
    | keyof typeof coordinatesContainer
    | typeof coordinatesContainer["rb"];
  pointContainer?:
    | keyof typeof coordinatesContainer
    | typeof coordinatesContainer["rb"];
  responsive?: boolean;
...
export const onTooltip = ({
  ...
  onMount(() => {
    const backTooltip = document.querySelector(
      `[data-modal-back-id="${containerId ?? modalId}"]`
    ) as HTMLDivElement;
    ...
    const calculatedOffset =
      containerOffsets +
      (typeof zoneOutOffsets === "boolean" ? 0 : zoneOutOffsets);
    ...
    const resizeEvent = (
      ...
      if (zoneOut) {
        const generateZoneElement = (
          ...
          if (!zone) {
            ...
            backTooltip.insertBefore(
              zone,
              backTooltip.querySelector('*:not([data-modal-type*="zone-"])')
            );
          }
        ...
        zone3.addEventListener("click", () => processOnZoneLeave(), {
          once: true,
        });
      ...
      if (processOnZoneLeave) {
        target.addEventListener("mouseleave", processOnZoneLeave, {
          once: true,
        });
      }
    };
