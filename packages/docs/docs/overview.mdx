...
## Demo
https://@nazfy/modal-next-example.nazfy.dev/
...
Para implementar un modal, inicia extendiendo tu componente de ModalProps para asegurar que tenga las propiedades requeridas. Posteriormente, utiliza la función generateModal de la biblioteca para crear un ModalRoot, que será el encargado de renderizar tu modal. Este componente ModalRoot no solo renderiza, sino que también provee al modal de propiedades especiales

#### Construye tu Modal
Tu primer elemento HTML que renderices deberias inyectarle las props de RootProps (Lo cual sera util luego para las animaciones). Las props que se ven aqui las va a proveer el futuro ModalRoot que renderizara nuestro modal.
...
```
<a href="/docs/container/vistas">
  :::tip
  Puedes ver mas en detalle como funcionan las vistas aqui.
  :::
</a>
