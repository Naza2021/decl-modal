...
export const { ToastRootProps, getToastContext, showToast: internalShow } = generateModal({ Modals: { Toast }, name: 'Toast' })

type ToastProps = ComponentProps<Toast>

export const showToast = (msg: ToastProps['content'], config: Omit<ToastProps, 'content'> = { timeout: 3000 }) => {
    return internalShow('Toast', { content: msg, ...config }, { override: false })
}